...
public class Desk {
    ...
    public void print() { // Prints out table
        int[] maxWidths = headers != null ? Arrays.stream(headers).mapToInt(String::length).toArray() : null; // Sets
                                                                                                              // header

        for (String[] cells : rows) { // Loops though cells
            if (maxWidths == null)
                maxWidths = new int[cells.length]; // Checks if headers and rows are the same size
            if (cells.length != maxWidths.length)
                throw new IllegalArgumentException("Error: Number of cells and headers must be the same.");
        ...
        for (String[] cells : rows)
            printRow(cells, maxWidths); // Prints all rows
        if (headers != null)
            printLine(maxWidths); // Prints end-cap
    }
    ...
    private void printLine(int[] columnWidths) { // Prints a line
        for (int i = 0; i < columnWidths.length; i++) {
            String line = String.join("", Collections.nCopies(columnWidths[i] + verticalSep.length() + 1, horzSep)); // Formats
                                                                                                                     // row
                                                                                                                     // on
                                                                                                                     // table
